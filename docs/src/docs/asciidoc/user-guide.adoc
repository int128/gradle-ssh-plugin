...
[NOTE]
.Difference of handling empty directories on put method
====
If a file filter is given, only matched files excluding empty directories will be put.
Otherwise, all files including empty directories will be put.
...
If no filter is given, all files and directories (same as above) will be put.

If the filter `{ it.name =~ /\.txt$/ }` is given, following files and directories will be put.
...
If the filter `{ it.name =~ /\.xml$/ }` is given, following files and directories will be put.
...
If the filter `{ it.name =~ /\.dat$/ }` is given, nothing will be put.
