...
try {
    ssh.run {
        session(ssh.remotes.tester) {
            ...
            execute 'git clone --depth 10 https://github.com/int128/gradle-ssh-plugin.git'
            execute "cd gradle-ssh-plugin && ./gradlew -Pgroovy-ssh-version=${System.getenv('TRAVIS_TAG') ?: 'SNAPSHOT'} check publishMavenPublicationToMavenLocal"
